...
```json
[{"pack_id":"01d84fd1-31dd-4b92-a942-c8e59ce38191","version":[1,2,29]}]
```

Resource pack:

```json
[{"pack_id":"834d72cc-8c7f-498f-adfa-269e215f86e8","version":[1,2,29]}]
```
...
**Updating an existing Fruity Friends installation:** replace the pack folders and `plum-service/bridge.py`, change both existing world-pack entries to `[1,2,29]`, and restart both processes. Preserve your current credentials/configuration. New plum, apple, blueberry, lemon, banana and grape trees appear only in newly generated terrain; explore new plains/forests (or warm biomes like deserts, savannas and jungles for lemons, jungles for bananas) or plant a Creative sapling in an old area.
...
To grow babies, plant a plum, apple, blueberry, lemon, banana or grape fruit on tilled farmland: a sprout appears and grows into a baby friend. Find the matching trees in new plains/forest terrain (grape vines here too; lemon trees in warm biomes like deserts, savannas and jungles, banana trees in jungles) and break their fruit-speckled leaves in Survival. Each leaf has a 35% fruit-drop chance and a separate 10% sapling-drop chance. Plant saplings on soil with a clear 5-wide, 6-high space; wait for random growth or use bone meal. Grape saplings only need 5-wide, 3-high and grow into a low leaf mound hiding a single log. Apples do not grow Plums. Leaves do not automatically decay.
