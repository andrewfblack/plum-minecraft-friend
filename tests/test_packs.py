...
class PackTests(unittest.TestCase):
    ...
    def test_lifecycle_references_and_baby_breeding(self):
        entity = json.loads((ROOT / 'packs/Plum_BP/entities/friend.json').read_text())['minecraft:entity']
        ...
        self.assertEqual(active, {'plum:baby'})
        self.assertNotIn('minecraft:breedable', groups['plum:baby'])
        apply('minecraft:on_tame')
        apply('minecraft:ageable_grow_up')
        self.assertEqual(active, {'plum:adult', 'plum:tamed'})
        self.assertEqual(groups['plum:adult']['minecraft:breedable']['breeds_with']['baby_type'], entity['description']['identifier'])

    def test_model_and_texture_references(self):
        rp = ROOT / 'packs/Plum_RP'
        client = json.loads((rp / 'entity/friend.entity.json').read_text())['minecraft:client_entity']['description']
        self.assertTrue((rp / (client['textures']['default'] + '.png')).exists())
        geo = json.loads((rp / 'models/entity/plum.geo.json').read_text())['minecraft:geometry'][0]
    ...
    def test_survival_fruit_chain_and_versioned_update(self):
        ...
        fruit = read(bp / 'items/plum.json')['minecraft:item']['description']['identifier']
        entity = read(bp / 'entities/friend.json')['minecraft:entity']
        self.assertEqual(entity['component_groups']['plum:adult']['minecraft:breedable']['breed_items'], [fruit])
        self.assertEqual(entity['component_groups']['plum:baby']['minecraft:ageable']['feed_items'], [fruit])
        self.assertIn(fruit, entity['components']['minecraft:behavior.tempt']['items'])
        tree = read(bp / 'features/plum_tree.json')['minecraft:tree_feature']
        rule = read(bp / 'feature_rules/plum_tree_rule.json')['minecraft:feature_rules']
        self.assertEqual(rule['description']['places_feature'], tree['description']['identifier'])
        leaves = read(bp / 'blocks/plum_leaves.json')['minecraft:block']
        ...
        self.assertEqual(drops, {fruit, 'plum:plum_sapling'})
        sapling = read(bp / 'blocks/plum_sapling.json')['minecraft:block']
        self.assertIn('plum:grow_tree', sapling['components']['minecraft:custom_components'])
        ...
        self.assertEqual(read(bp / 'manifest.json')['header']['version'], [1, 1, 0])
        with zipfile.ZipFile(ROOT / 'dist/Plum-Dedicated-Server.zip') as z:
            self.assertEqual(json.loads(z.read('world-pack-lists/world_behavior_packs.json'))[0]['version'], [1, 1, 0])
